    ...
    public function alipayReturnHandle($type, $data)
    {
        switch ($type){
            ...
            case 'pub task':
                ...
                $url = 'task/tasksuccess/';
                if($task['status'] == 2){
                    $url = 'task/tasksuccess/';
                }

                if(!empty($task_id) && $task_id['status'] == 0){
                ...
                return redirect()->to($url . $task_id['task_id'].'?a=04');
...
//                                UserDetailModel::where('uid', Auth::id())->decrement('balance', $orderInfo->cash);
                ...
                break;
            case 'stylist':
                $waitHandle = StylistOrderModel::where('code', $data['code'])->first();
                if (!empty($waitHandle)){
                    switch ($waitHandle->status){
                        case 0:
                            $status = DB::transaction(function () use ($data) {
                                $orderInfo = StylistOrderModel::where('code', $data['code'])->first();
//                                UserDetailModel::where('uid', Auth::id())->decrement('balance', $orderInfo->cash);
                                FinancialModel::create([
                                    'action' => 16,
                                    'pay_type' => 2,
            ...
            case 'task service':
                ...
                break;
            case 'brain power':
                $res = BrainpowerOrderModel::where('code', $data['code'])->first();
                if (!empty($res) && $res->status == 0) {
                    $BrainpowerOrderModel = new BrainpowerOrderModel();
                    $status = $BrainpowerOrderModel->pay('alipay', $data);
                    FinancialModel::create([
                        'action' => 12,
                        'pay_type' => 2,
                        'cash' => $res->cash,
        ...
        switch ($type){
            ...
            case 'pub task':
                ...
                if(!empty($task_id)){
                    ...
                    $status = UserDetailModel::recharge($task_id['uid'], 3, $data);
                    if (!$status) {
                        echo '支付失败！';
                    }
                    $task = TaskModel::find($task_id['task_id']);
