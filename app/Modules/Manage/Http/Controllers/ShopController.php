...
use App\Modules\Shop\Models\GoodsSlideshowModel;
...
//    平台商铺轮播图
    public function goodsSlideshow (Request $request){
        $this->theme->setTitle('平台商铺轮播图');
        $merge = $request->all();
        $goodsList = GoodsSlideshowModel::whereRaw('1 = 1');
        if ($request->get('name')) {
            $goodsList = $goodsList->where('name','like','%'. $request->get('name').'%');
        }
        if ($request->get('status')) {
            ...
            $goodsList = $goodsList->where('status', $status);
        }
        ...
        return $this->theme->scope('manage.goodsslideshow', $data)->render();
    }
//平台商铺轮播图添加
    public function goodsSlideshowAdd(Request $request){
        $this->theme->setTitle('平台商铺轮播图添加');
        if ($request->isMethod('post')){
            ...
            if ($url){
                $uploadMsg = json_decode(\FileClass::uploadFile($url, 'user', $allowExtension));
                if ($uploadMsg->code != 200) {
                    $error['card_front_side'] = $uploadMsg->message;
                } else {
                    $arr['url'] = $uploadMsg->data->url;
                }
            }
            $res = DB::table('goods_slideshow')->insert(
                 ['name' => $data['name'],
                     'url' => $arr['url'],
                     'status' => $data['status'],
                     'sort' => $data['sort'],
            ...
            if($res){
                return redirect('/manage/goodsSlideshowAdd')->with(array('message' => '操作成功'));
            }
            return redirect('/manage/goodsSlideshowAdd')->with(array('message' => '操作失败'));
        }
        return $this->theme->scope('manage.goodsslideshowadd')->render();
    }


//    平台商铺轮播图查看修改
    public function goodsSlideshowInfo($id){
        $this->theme->setTitle('平台商铺轮播图查看修改');
        $id = intval($id);
        $goodsslideshowinfo = DB::table('goods_slideshow')->where('id',$id)->first();
        $data = [
            'goods' => $goodsslideshowinfo
        ];
        return $this->theme->scope('manage.goodsslideshowinfo', $data)->render();
    }
//    平台商铺轮播图查看修改提交
    public function goodsSlideshowInfosave(Request $request){
        $data = $request->except('_token');
        $url = $request->file('url');
        $allowExtension = array('jpg', 'gif', 'jpeg', 'bmp', 'png','JPG', 'GIF', 'JPEG', 'BMP', 'PNG');
        if ($url){
        ...
        if($res){
            return redirect('/manage/goodsSlideshowInfo/'.$data['id'])->with(array('message' => '操作成功'));
        }
        return redirect('/manage/goodsSlideshowInfo/'.$data['id'])->with(array('message' => '操作失败'));
    }
