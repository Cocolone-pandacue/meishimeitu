...
class TaskRightsController extends ManageController
{
    ...
    public function handleRights(Request $request)
    {
        ...
        $trading_rights_result  = MessageTemplateModel::where('code_name','trading_rights_result')->where('is_open',1)->first();
        if($trading_rights_result)
        {
            ...
            if($trading_rights_result->is_on_site == 1){
                \MessageTemplateClass::getMeaasgeByCode('trading_rights_result',$owner_user['id'],2,$ownerMessageVariableArr,$trading_rights_result['name']);
            }

            if($trading_rights_result->is_send_email == 1){
                $email = $owner_user->email;
                \MessageTemplateClass::sendEmailByCode('trading_rights_result',$email,$ownerMessageVariableArr,$trading_rights_result['name']);
            }
            ...
            if($trading_rights_result->is_on_site == 1){
                \MessageTemplateClass::getMeaasgeByCode('trading_rights_result',$worker_user['id'],2,$workerMessageVariableArr,$trading_rights_result['name']);
            }

            if($trading_rights_result->is_send_email == 1){
                $email = $worker_user->email;
                \MessageTemplateClass::sendEmailByCode('trading_rights_result',$email,$workerMessageVariableArr,$trading_rights_result['name']);
            }
        }
