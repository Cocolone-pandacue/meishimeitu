    ...
    static public function phpSmsConfig($alias)
    {
        ...
        switch($alias){
            ...
            if($config && !empty($config['rule'])){
                ...
                break;
            case 'sendStylistApply':                //设计师申请项目模板
                $rule = $config['rule'];
                break;
            case 'sendAcceptnew':                //雇佣接受通知模板
                $rule = $config['rule'];
                break;
            case 'sendStylistAccept':                //设计师接受任务通知模板
                $rule = $config['rule'];
                break;
            case 'sendStylistRefuse':                //设计师拒绝任务通知模板
                $rule = $config['rule'];
                break;
            case 'sendStylistFile':                //设计师上传稿件模板
                $rule = $config['rule'];
                break;
            case 'sendManuscriptSettlementNew':                //项目结款模板
                $rule = $config['rule'];
                break;
        }
        return $rule;
