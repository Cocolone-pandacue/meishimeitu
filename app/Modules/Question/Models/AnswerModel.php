...
class AnswerModel extends Model
{
    ...
    static public function adopt($adoptid,$questionid)
    {
        ...
        if(is_null($status))
        {
            ...
            $question_accept = MessageTemplateModel::where('code_name','question_accept')->where('is_open',1)->first();
            if($question_accept)
            {
                ...
                if($question_accept->is_on_site == 1){
                    \MessageTemplateClass::getMeaasgeByCode('question_accept',$answeruser['id'],2,$messageVariableArr,$question_accept['name']);
                }

                if($question_accept->is_send_email == 1){
                    $email = $answeruser->email;
                    \MessageTemplateClass::sendEmailByCode('question_accept',$email,$messageVariableArr,$question_accept['name']);
                }
            }
        }
        return $status;
