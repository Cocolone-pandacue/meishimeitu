...
//        家乡
        if (!is_null($uinfo['province_home'])) {
            $city_home = DistrictModel::findTree($uinfo['province_home']);
        } else {
            $city_home = DistrictModel::findTree($province_home[0]['id']);
        }

        if (!is_null($uinfo['city_home'])) {
            $area_home = DistrictModel::findTree($uinfo['city_home']);
        } else {
        ...
        $sign = json_decode($uinfo->sign);
        $signvalue = '';
        if (is_array($sign)){
            foreach ($sign as $s){
                $signvalue[] = DB::table('cate')->where('id',$s)->first();
            }
        }
//        dd($signvalue);die;
        $view = array(
...
//    个人资料扩展修改
    public function infoExtendUpdate(Request $request){
...
//    我的会员
    public function member(){
        $this->initTheme('userinfo');
        $this->theme->setTitle('会员中心');
        $uid = $this->user['id'];
        $stylistPackageOrder = StylistPackageOrderModel::where('uid',$uid)->where('status',0)->first();
        $stylistPackage = null;
        if ($stylistPackageOrder){
            $stylistPackage = StylistPackageModel::where('id',$stylistPackageOrder->package_id)->first();
        }
//        dd($stylistPackageOrder);
        $view = array(
            'stylistPackageOrder' => $stylistPackageOrder,
            'stylistPackage' => $stylistPackage,
        );
        return $this->theme->scope('user.member', $view)->render();
    }
    ...
    public function download(){
        $this->initTheme('userinfo');
        $this->theme->setTitle('作品下载');
        $view = array(

        );
        return $this->theme->scope('user.download', $view)->render();
    }
//我的下载智能工具
    public function DownloadBrainpower(){
        $this->initTheme('userinfo');
        $this->theme->setTitle('辅助工具');
        $brainpower = DB::table('brainpower_order as bo')
            ->select('bo.*','bul.svg')
            ->where('bo.uid',$this->user['id'])
            ->where('bo.status',1)
            ->join('brainpower_user_logo as bul','bo.logo_id','=','bul.id')
            ->orderBy('updated_at', 'desc')
            ->paginate(9);
        $brainpower_data = $brainpower->toArray();
        $brainpowernum = DB::table('brainpower_order')
            ->select('logo_id')
            ->where('uid',$this->user['id'])
            ->where('status',1)
            ->count();
//        dd($brainpowernum);die;
        $view = array(
            'brainpower' => $brainpower,
            'brainpower_data' => $brainpower_data,
            'brainpowernum' => $brainpowernum,
        );
        return $this->theme->scope('user.downloadbrainpower', $view)->render();
    }
