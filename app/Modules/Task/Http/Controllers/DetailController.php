...
class DetailController extends IndexController
{
	...
	public function index($id,Request $request)
    {
        ...
        if($detail['region_limit']==1)
        {
            $areaArr = [];
            if($detail['province']){
                array_push($areaArr,$detail['province']);
            }
            if($detail['city']){
                array_push($areaArr,$detail['city']);
            }
            if($detail['area']){
                array_push($areaArr,$detail['area']);
            }
            $province = DistrictModel::whereIn('id',$areaArr)->get()->toArray();
            $province = \CommonClass::keyBy($province,'id');
            $view['area'] = $province;
        }
//        行业管理
        if($detail['industry']){
            $industry = DB::table('industry')->where('id', $detail['industry'])->first();
            $view['industry'] = $industry->name;
        }
//        参考产品
        if($detail['reference']){
...
//        设计风格
        if($detail['task_style_id']){
            $task_style = DB::table('style_task')->where('task_style_id', $detail['task_style_id'])->lists('style_id');
            // $task_style=implode(",", $task_style);
            $view['task_style'] = $task_style;
            $view['style_conf'] = [
                1 => '简约',
                2 => '华丽',
                3 => '古典',
...
//        颜色
        if ($detail['task_tonality_id']){
            $task_tonality = DB::table('tonality_task')->where('task_tonality_id', $detail['task_tonality_id'])->lists('tonality_id');
            // $task_tonality=implode(",", $task_tonality);
            $view['task_tonality'] = $task_tonality;
            $view['tonality_conf'] = [
                1 => '蓝色调',
                2 => '青色调',
                3 => '绿色调',
...
//        dd($view);die;
    ...
    public function workCreate(WorkRequest $request)
    {
        ...
        $task_delivery = MessageTemplateModel::where('code_name','task_delivery')->where('is_open',1)->first();
        if($task_delivery)
        {
            ...
            if($task_delivery->is_on_site == 1){
                \MessageTemplateClass::getMeaasgeByCode('task_delivery',$user['id'],2,$messageVariableArr,$task_delivery['name']);
            }

            if($task_delivery->is_send_email == 1){
                $email = $user->email;
                \MessageTemplateClass::sendEmailByCode('task_delivery',$email,$messageVariableArr,$task_delivery['name']);
            }
...
//    申请项目ajax
    public function myTasksAsk(Request $request){
        $data = $request->all();
        $uid = Auth::user()['id'];
        $query = WorkModel::where('task_id',$data['id'])->where('uid',$uid)->get()->toArray();
        $code = 0;
        if ($query == []){
            $work = WorkModel::insert([
                'task_id' => $data['id'],
                'status' => 0,
                'uid' => $uid,
...
//          发送申请信息
            ...
            $user = UserModel::where('id', $task['uid'])->first();
            $site_name = \CommonClass::getConfig('site_name');
            $stylist = UserModel::where('id', $uid)->first();
//            $stylist = UserModel::where('id', $stylist_user['uid'])->first();
            $task_audit_failure = MessageTemplateModel::where('code_name', 'stylist_apply')->where('is_open', 1)->first();
            if ($task_audit_failure){
                $messageVariableArr = [
                    'username' => $user['name'],
                    'stylist_name' => $stylist['name'],
                    'task_name' => $task['title'],
//                    'phone' => $stylist['mobile'],
                    'website' => $site_name,
...
//                    'stylist_name' => $stylist['name'],
                ...
                if($task_audit_failure->is_on_site == 1){
                    \MessageTemplateClass::getMeaasgeByCode('stylist_apply',$task['uid'],2,$messageVariableArr,$task_audit_failure['name'],$uid);
                }
...
//        发送短信信息
            ...
            $templateId = ConfigModel::phpSmsConfig('sendStylistApply');
            $templates = [
                $scheme => $templateId,
            ];
            $tempData = [
...
//          \SmsClass::sendSms(手机号, 短信模板配置项, 传的变量);
            $status = \SmsClass::sendSms($user['mobile'], $templates, $tempData);
...
//    项目结款
    public function workCheckId($workid){
        $data['work_id'] = $workid;
//        $data['work_status'] = 3;
        $work_data = WorkModel::where('id',$workid)->first();
        $data['uid'] = $work_data['uid'];
        if(!TaskModel::isEmployer($work_data['task_id'],$this->user['id']))
            return redirect()->back()->with(['error'=>'您不是雇主，您的操作有误！']);
//        if($work_data['status']!=4 || $work_data['status']!=5)
//            return redirect()->back()->with(['error'=>'当前稿件不具备验收资格！']);
        $worker_num = TaskModel::where('id',$work_data['task_id'])->first();
        $worker_num = $worker_num['worker_num'];

        $win_check = WorkModel::where('work.task_id',$workid)->where('status','>',5)->count();
        $data['worker_num'] = $worker_num;
        $data['win_check'] = $win_check;
        $data['task_id'] = $work_data['task_id'];
        $workModel = new WorkModel();
...
//    项目结款ajax
    public function workCheckIdAjax(Request $request){
        $data = $request->except('_token');
        $data['fuid'] = $this->user['id'];
        $workid = $data['work_id'];
//        $data['work_id'] = $workid;
//        $data['work_status'] = 3;
        $work_data = WorkModel::where('id',$workid)->first();
...
//        上线解开注释
        if (!in_array($work_data['status'], [4,5])){
            return redirect()->back()->with(['error'=>'当前稿件不具备验收资格！']);
        }
...
//        if($result) return redirect()->to('task/'.$data['task_id'])->with(['manage'=>'验收成功！']);
    ...
    public function ajaxRights(Request $request)
    {
        ...
        $trading_rights = MessageTemplateModel::where('code_name','trading_rights')->where('is_open',1)->first();
        if($trading_rights)
        {
            ...
            if($trading_rights->is_on_site == 1){
                \MessageTemplateClass::getMeaasgeByCode('trading_rights',$from_user['id'],2,$fromMessageVariableArr,$trading_rights['name']);
            }

            if($trading_rights->is_send_email == 1){
                $email = $from_user->email;
                \MessageTemplateClass::sendEmailByCode('trading_rights',$email,$fromMessageVariableArr,$trading_rights['name']);
            }
    ...
    public function report(Request $request)
    {
        ...
        $task_publish_success = MessageTemplateModel::where('code_name','report')->where('is_open',1)->first();
        if($task_publish_success)
        {
            ...
            if($task_publish_success->is_on_site == 1){
                \MessageTemplateClass::getMeaasgeByCode('report',$this->user['id'],1,$messageVariableArr,$task_publish_success['name']);
            }

            if($task_publish_success->is_send_email == 1){
                $email = $this->user['email'];
                \MessageTemplateClass::sendEmailByCode('report',$email,$messageVariableArr,$task_publish_success['name']);
            }
