...
class WorkAttachmentModel extends Model
{
    ...
    //    项目的附件
    public static function myWorkAttachment($workid){
        $query = Self::select('work_attachment.id as workattid','att.*')
            ->where('work_attachment.work_id',$workid)
            ->join('attachment as att','att.id','=','work_attachment.attachment_id')
            ->get()->toArray();
        return $query;

    }
    //    项目的附件
    public static function myTaskAttachment($taskid){
        $query = Self::select('work_attachment.id as workattid','att.*','work_attachment.work_id')
            ->where('work_attachment.task_id',$taskid)
            ->join('attachment as att','att.id','=','work_attachment.attachment_id')
            ->orderBy('work_attachment.created_at', 'desc')
            ->get()->toArray();
        return $query;
