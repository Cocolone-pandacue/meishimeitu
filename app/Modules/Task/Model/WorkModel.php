...
//    查找uid被选中的项目id
    public static function findwork($uid){
        $query = Self::select('task_id')->where('uid',$uid)->whereIn('status',[0,1])->get()->toArray();
        return $query;
    }

//    申请项目的人
    public static function myWork($taskid){
        $query = Self::select('work.task_id','ud.avatar','di.name as province','pr.profession','ud.uid','us.name','work.status')
            ->where('work.task_id',$taskid)
            ->whereIn('work.status',[0,1])
            ->leftjoin('users as us','us.id','=','work.uid')
            ->leftjoin('user_detail as ud','ud.uid','=','work.uid')
            ->leftjoin('district as di','ud.province','=','di.id')
            ->leftjoin('profession as pr','pr.id','=','ud.profession_id')
            ->get()->toArray();
        if ($query){
            foreach($query as $q){
                if ($q['status'] ==1){
...
//    承接项目
    public static function myWorks($data){
        $query = self::select('work.*','ta.bounty','ta.desc','ta.title','tc.name as cate_name','in.name as industry_name')
            ->where('work.status', '>=', 0)
            ->where('work.status', '!=', 8)
            ->where('work.status', '<', 8)
...
//        dd($query);
        if (isset($data['status']) && $data['status'] != 0) {
            ...
            $query->whereIn('work.status', $status);
        }
        if (isset($data['count']) && $data['count'] == 0){
            $data = count($query                //统计发布项目个数参数
                ->leftjoin('task as ta','ta.id','=','work.task_id')
                ->leftjoin('cate as tc','tc.id', '=','ta.cate_id')
                ->leftjoin('industry as in','in.id','=','ta.industry')
    ...
    static public function workCheckold($data)
    {
        $status = DB::transaction(function() use($data) {
        ...
        if(is_null($status))
        {
            ...
            $manuscript_settlement = MessageTemplateModel::where('code_name','manuscript_settlement')->where('is_open',1)->first();
            if($manuscript_settlement)
            {
                ...
                if($manuscript_settlement->is_on_site == 1){
                    \MessageTemplateClass::getMeaasgeByCode('manuscript_settlement',$user['id'],2,$messageVariableArr,'任务验收通知');
                }

                if($manuscript_settlement->is_send_email == 1){
                    $email = $user->email;
                    \MessageTemplateClass::sendEmailByCode('manuscript_settlement',$email,$messageVariableArr,'任务验收通知');
                }
            }
        }
        return is_null($status)?true:false;
...
//    项目借款新
    static public function workCheck($data)
    {
        $status = DB::transaction(function() use($data) {
            ...
            self::where('id', $data['work_id'])->update(['status' => 6, 'bid_at' => date('Y-m-d H:i:s', time())]);
        ...
        if(is_null($status))
        {
            ...
            $manuscript_settlement = MessageTemplateModel::where('code_name','manuscript_settlement_new')->where('is_open',1)->first();
            if($manuscript_settlement)
            {
                $task = TaskModel::where('id',$data['task_id'])->first();
                $work = WorkModel::where('id',$data['work_id'])->first();
                $user = UserModel::where('id',$work['uid'])->first();
                $fuser = UserModel::where('id',$data['fuid'])->first();
                $site_name = \CommonClass::getConfig('site_name');
                $domain = \CommonClass::getDomain();
                ...
                $messageVariableArr = [
                    ...
                    'stylist_name'=>$fuser['name'],
//                    'phone'=>$fuser['mobile'],
                    'website'=>$site_name,
//                    'task_link'=>$domain.'/task/'.$task['id'],
                    'task_link'=>$domain.'/finance/assetDetail?type=2',

                ];
                if($manuscript_settlement->is_on_site == 1){
                    \MessageTemplateClass::getMeaasgeByCode('manuscript_settlement_new',$user['id'],2,$messageVariableArr,'任务验收通知',$data['fuid']);
                }

                if($manuscript_settlement->is_send_email == 1){
                    $email = $user->email;
                    \MessageTemplateClass::sendEmailByCode('manuscript_settlement_new',$email,$messageVariableArr,'任务验收通知');
                }
//              发送短信信息
                $scheme = ConfigModel::phpSmsConfig('phpsms_scheme');
                $templateId = ConfigModel::phpSmsConfig('sendManuscriptSettlementNew');
                $templates = [
                    $scheme => $templateId,
                ];
                $tempData = [
...
//          \SmsClass::sendSms(手机号, 短信模板配置项, 传的变量);
                $statusnew = \SmsClass::sendSms($user['mobile'], $templates, $tempData);
            }
        }
        return is_null($status)?true:false;
    }
    ...
    static public function sendTaskWidMessage($arr)
    {
        ...
        $task_win = MessageTemplateModel::where('code_name','task_win')->where('is_open',1)->first();
        if($task_win)
        {
            ...
            if($task_win->is_on_site == 1){
                \MessageTemplateClass::getMeaasgeByCode('task_win',$user['id'],2,$messageVariableArr,'任务中标通知');
            }

            if($task_win->is_send_email == 1){
                $email = $user->email;
                \MessageTemplateClass::sendEmailByCode('task_win',$email,$messageVariableArr,'任务中标通知');
            }
    ...
    static public function bidWorkCheck($data)
    {
        if($data['status'] == 1){
            ...
            if(is_null($status))
            {
                ...
                $manuscript_settlement = MessageTemplateModel::where('code_name','bid_work_check_success')->where('is_open',1)->first();
                if($manuscript_settlement)
                {
                    $task = TaskModel::where('id',$data['task_id'])->first();
                    $work = WorkModel::where('id',$data['work_id'])->first();
                    ...
                    if($manuscript_settlement->is_on_site == 1){
                        \MessageTemplateClass::getMeaasgeByCode('bid_work_check_success',$user['id'],2,$messageVariableArr,$manuscript_settlement['name']);
                    }

                    if($manuscript_settlement->is_send_email == 1){
                        $email = $user->email;
                        \MessageTemplateClass::sendEmailByCode('bid_work_check_success',$email,$messageVariableArr,$manuscript_settlement['name']);
                    }
                }
            }
        }else{
            ...
            if(is_null($status))
            {
                ...
                $manuscript_settlement = MessageTemplateModel::where('code_name','bid_work_check_failure')->where('is_open',1)->first();
                if($manuscript_settlement)
                {
                    $task = TaskModel::where('id',$data['task_id'])->first();
                    $work = WorkModel::where('id',$data['work_id'])->first();
                    ...
                    if($manuscript_settlement->is_on_site == 1){
                        \MessageTemplateClass::getMeaasgeByCode('bid_work_check_failure',$user['id'],2,$messageVariableArr,$manuscript_settlement['name']);
                    }

                    if($manuscript_settlement->is_send_email == 1){
                        $email = $user->email;
                        \MessageTemplateClass::sendEmailByCode('bid_work_check_failure',$email,$messageVariableArr,$manuscript_settlement['name']);
                    }
