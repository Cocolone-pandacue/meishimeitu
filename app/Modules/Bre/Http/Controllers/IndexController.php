...
class IndexController extends \App\Http\Controllers\IndexController
{
    ...
    public function getService(Request $request)
    {
		...
		$NavName= \CommonClass::getNavName('/bre/service');
		if(!$NavName){
			$NavName="服务商大厅";
		}
...
//作品详情
    public function buyGoodsNew(Request $request, $id){
        $this->theme->setTitle('作品详情');
        $id = intval($id);
        $data = $request->all();
        $view_num = DB::table('goods')->where('id',$id)->select('view_num')->first();
        DB::table('goods')->where('id',$id)->update(['view_num' => $view_num->view_num+1]);
        $goodsinfo = DB::table('goods')
            ->join('users', 'goods.uid', '=', 'users.id')
            ->join('cate', 'goods.cate_id', '=', 'cate.id')
            ->select('goods.*', 'users.name as usersname','cate.name as catename','cate.pid as catepid')
            ->where('goods.id',$id)
            ->first();
//        dd($goodsinfo);die;
        $cate = DB::table('cate')->where('id',$goodsinfo->catepid)->select('name')->first();
        $data = [
            'goodsinfo' => $goodsinfo,
            'cate' => $cate
        ];
        return $this->theme->scope('bre.buygoodsnew', $data)->render();
    }


//    平台商铺
    public function shop(Request $request){
        $this->theme->setTitle('平台商铺');
        $data = $request->all();
        $goodsslideshow = DB::table('goods_slideshow')
            ->where('status',1)
            ->orderBy('sort','asc')
            ->orderBy('updated_at','desc')
            ->get();
        $paginate = $request->get('paginate') ? $request->get('paginate') : 25;
        $goodsinfo = DB::table('goods')
            ->join('users', 'goods.uid', '=', 'users.id')
            ->join('cate', 'goods.cate_id', '=', 'cate.id')
            ->join('user_detail', 'goods.uid', '=', 'user_detail.uid')
            ->select('goods.*', 'users.name as usersname','cate.name as catename','cate.pid as catepid','user_detail.avatar')
        ...
        if (isset($data['category']) && $data['category']!=0 ) {
            ...
            if (empty($category)) {
                $categorytow = DB::table('cate')->where('id',$data['category'])->select('name','pid')->first();
                if ($categorytow){
                    $categoryone = DB::table('cate')->where('id',$categorytow->pid)->select('name')->first();
                ...
                $categoryone = DB::table('cate')->where('id',$data['category'])->select('name')->first();
            }
        }
        ...
        $cateone = 0;
        if(isset($data['cate']) && !isset($data['category'])){
            foreach ($data['cate'] as $d => $a){
                $category[] = TaskCateModel::findByPid([intval($a)]);
            }
            $cateone = DB::table('cate')->whereIn('id',$data['cate'])->select('name')->get();
        ...
        switch($type){
            ...
            case '2':
                ...
                break;
            case '3':
                $goodsinfo = $goodsinfo->orderBy('created_at','desc');
                break;
            default: 
                $goodsinfo = $goodsinfo->orderBy('sales_num','desc')
                    ->orderBy('view_num','desc');
        ...
        $goodsinfo = $goodsinfo->paginate($paginate);
        ...
        // dd($goodsinfo);die;
        $data = [
            'goodsslideshow' => $goodsslideshow,
            'goodsinfo' => $goodsinfo,
            'goods_focus' => $goods_focus,
            'cateall' => $cateall,
            'merge' => $data,
            'categorytow' => $categorytow,
...
//    废弃原平台商铺
    public function shopold(Request $request)
    {
        $this->theme->setTitle('平台商铺');
        $data = $request->all();
        $workInfo = $uid = [];
