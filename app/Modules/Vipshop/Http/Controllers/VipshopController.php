...
//新的vip主页
    public function Index(){
        $this->theme->setTitle('vip专享服务');
        $stylist_package = StylistPackageModel::where('status', 0)
//            ->orderBy('list', 'asc')
...
//        dd($stylist_package);
        $data= [
            'stylist_package' => $stylist_package,
        ];
        return $this->theme->scope('vipshop.index', $data)->render();
    }
...
//    post接受设计师套餐参数
    public function postPayvipNew(Request $request){
        $data = $request->all();
        $stylist = StylistAuthModel::where('uid', Auth::id())->orderBy('created_at','desc')->where('status', '1')->first();
        if (empty($stylist)) {
            return back()->with(['message' => '请先入驻设计师']);
        }
        $havePackage = StylistPackageOrderModel::where('uid',Auth::id())->where('status', 0)->first();
        if ($havePackage) {
            return back()->with(['message' => '你已购买套餐']);
        }
        $package = StylistPackageModel::where('id',$data['packag_id'])->first()->toArray();
        $cash = $package['cash']*$package['stylist_success_draw_ratio'];
        $code = ShopOrderModel::randomCode(Auth::id(), 'st');
//        dd($package);
        $data = [
            'code' => $code,
            'cash' => $cash,
            ...
            'package_id' => $data['packag_id'],
            'time_period' => $package['time']
        ];
        $status = StylistOrderModel::create($data);
        if ($status) {
//            session(['vipshopcode' => $status->code]);
            return redirect('vipshop/vipPayorder/'.$status->code);
        }
    }
...
//    接单凭证付款
    public function vipPayorderNew($code){
        $vipcode = $code;
        $orderInfo = StylistOrderModel::where('code', $vipcode)->firstOrFail();
        ...
        return $this->theme->scope('vipshop.vipPayorderNew', $data)->render();
    }
...
//余额支付
    public function postVipPayorderNew(PayOrderRequest $request){
        $data = $request->except('_token');
//        dd($data);
        $userInfo = UserModel::find(Auth::id());
        $password = UserModel::encryptPassword($data['password'], $userInfo->salt);
        $havePackage = StylistPackageOrderModel::where('uid', Auth::id())->where('status', 0)->first();
        if ($havePackage) return back()->with(['message' => '你已购买次套餐']);
        if ($password == $userInfo->alternate_password) {
            $vipcode = $data['code'];
            $status = DB::transaction(function () use ($vipcode) {
                $orderInfo = StylistOrderModel::where('code', $vipcode)->first();
                UserDetailModel::where('uid', Auth::id())->decrement('balance', $orderInfo->cash);
                FinancialModel::create([
                    'action' => 16,
                    'pay_type' => 1,
                ...
                StylistOrderModel::where('code', $orderInfo->code)->update(['status' => 1]);
                StylistPackageOrderModel::create([
                    'stylist_id' => $orderInfo->stylist_id,
                    'package_id' => $orderInfo->package_id,
                    'uid' => Auth::id(),
                    'time_period' => $orderInfo->time_period,
                    'cash' => $orderInfo->cash,
                    'start_time' => date('Y-m-d H:i:s', time()),
                    'end_time' => date('Y-m-d H:i:s', strtotime('+' . $orderInfo->time_period . ' day')),
                    'status' => 0
                ]);
            });
...
//第三方支付
    public function thirdPayorderNew(Request $request){
//        $data = $request->except('_token');
        $type = Input::get('pay_type');
        $vipcode = Input::get('code');
        $vipOrder = StylistOrderModel::where('code', $vipcode)->first();
        switch ($type) {
            case 'alipay':
                $config = ConfigModel::getPayConfig('alipay');
                $objOminipay = Omnipay::gateway('alipay');
