...
class TaskSelectWork extends Command
{
    ...
    private function expireTasks($data)
    {
        ...
        foreach($data as $v)
        {
            if((strtotime($v['selected_work_at'])+$task_select_work*24*3600)<=$time)
            {
                $expireTasks[] = $v['id'];
            }
        }
    ...
    private function sendMassage ($task_id)
    {
        ...
        foreach($ids as $v)
        {
            $task_publish_success = MessageTemplateModel::where('code_name','Automatic_choose')->where('is_open',1)->first();
            if($task_publish_success)
            {
                ...
                if($task_publish_success->is_on_site == 1){
                    \MessageTemplateClass::getMeaasgeByCode('Automatic_choose',$user['id'],2,$messageVariableArr,$task_publish_success['name']);
                }

                if($task_publish_success->is_send_email == 1){
                    $email = $user->email;
                    \MessageTemplateClass::sendEmailByCode('Automatic_choose',$email,$messageVariableArr,$task_publish_success['name']);
                }
            }
        }
