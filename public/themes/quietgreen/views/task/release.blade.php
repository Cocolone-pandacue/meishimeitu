                                        ...
                                        <span id="area_select" style="{{ ($task['region_limit']==1)?'':'display:none' }}">
                                            ...
                                            <span class="help-inline col-xs-12 col-sm-7">
                                                <label class="middle">
                                                    ...
                                                    <span class="lbl txt-lbl"><a href="/task">逛项目库看看别人怎么写</a></span>
                                                </label>
                                            </span>
                                            ...
                                            {{ (!empty(old('xuanshang')))?'in':'' }} @if($task['type_id'] == $type_v['id'])in active @endif" id="ck">
                                                        ...
                                                        <span class="input-group-addon date-icon" >
                                                            <i class="fa fa-calendar bigger-110"></i>
                                                                    ...
                                                                    <div>
                                                                        <input type="number" class="input-sm form-control " id="datepiker-begin" onchange="beginAt($(this))" @if($task['type_id'] == $type_v['id']) value="{!! ($task['begin_at'])?date('Y年m月d日',strtotime($task['begin_at'])):date('Y年m月d日',time()) !!}" @else value="{{date('Y年m月d日',time())}}" @endif placeholder="开始时间" >
                                                                    </div>
                                                        <span class="input-group-addon  date-icon ">
...
name="delivery_deadline{{$type_v['alias']}}" ajaxurl="/task/checkDeadlineByBid"  datatype="*" nullmsg="请填写截止时间！" @if($task['type_id'] == $type_v['id']) value="{!! ($task['delivery_deadline'])?date('Y年m月d日',strtotime($task['delivery_deadline'])):date('Y年m月d日',time()) !!}" @endif class="delivery_deadline"/>
                                                    <span class="time_check validform_checktip"
                                                          style="width:90%;margin-left:20px;display:none">
                                                                ...
                                                                <div>
                                                                    <input type="text" name="bounty{{$type_v['alias']}}" class="mis-txt" @if($task['type_id'] == $type_v['id'] && $task['bounty'] > 0)value="{{floatval($task['bounty'])}}" @endif
