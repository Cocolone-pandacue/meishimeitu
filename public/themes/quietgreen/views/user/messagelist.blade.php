...
<div class="message">
    <ul class="left_tips">
        <p class="left_tips_all">全部</p>
        <li @if($type == 1) class="sele_tip" @else class="" @endif">
            <a class="ho_color1" href="/user/messageList/1">系统消息</a>
        </li>
        <li @if($type == 2) class="sele_tip" @else class="" @endif">
            <a class="ho_color1" href="/user/messageList/2">交易动态</a>
        </li>
        <li @if($type == 4) class="sele_tip" @else class="" @endif">
            <a class="ho_color1" href="/user/messageList/4">收件箱</a>
        </li>
        <li @if($type == 3) class="sele_tip" @else class="" @endif">
            <a class="ho_color1" href="/user/messageList/3">发件箱</a>
        </li>
    </ul>
    <div class="message_right">
        <div class="mydownload_fenlei">
            <p class="mydownload_biaoti">
                <span><img src="{!! Theme::asset()->url('images/Finances.png') !!}" alt="" class="mydownload_biaoti_photo"></span>
                <span class="mydownload_biaoti_text">信息</span>
            </p>
            <div class="mydownload_xifen_leixing">
                <a href="javascript:;" class="all_message clickyed">全部信息</a>
                <a href="javascript:;" class="no_read">未读信息(<span style="padding-right: 0">{{$new_count}}</span>)</a>
                <a href="javascript:;" id="is_read" >已读信息</a>
            </div>
            <p class="mydownload_xifen_zhuangtai ageshi">
                <a class="all_pick all_in">全选</a>
                <a class="is_read">标记所选为已读</a>
                <a class="is_delete">删除所选</a>
            </p>
        </div>
        <div class="g-main g-message">
            <h4 class="text-size16 u-title text_bold border_bottom add_height" data-values="{{$type}}">@if($type == 1)系统消息 @elseif($type == 3)
                    发件箱 @elseif($type == 4) 收件箱 @else 交易动态 @endif</h4>
            @if(!empty($message) && !empty($message->toArray()['data']))
            <!-- <div class="space-4"></div>
            <div class="well clearfix hidden-xs">
