<div class="accordion-style1 panel-group accordion-style2 g-side1" id="accordion">
    <div class="panel panel-default">
        <div class="panel-heading">
            <h4 class="panel-title clearfix">
                <a href="#collapseThree2"  data-parent="#accordion" data-toggle="collapse" class="accordion-toggle g-wrap1
                {{ (preg_match('/^\/user\/(productionUploading|productionFeatedNew)/',$_SERVER['REQUEST_URI']))?'g-active':'' }}">
                    <i class="text-size20 g-tradingico"></i>&nbsp;&nbsp;&nbsp;&nbsp;入驻申请
                    <i class="pull-right ace-icon fa fa-angle-right" data-icon-hide="fa-angle-down" data-icon-show="fa-angle-right"></i>

                </a>
            </h4>
        </div>
        <div id="collapseThree2" class="panel-collapse  collapse {{ (preg_match('/^\/user\/(productionUploading|productionFeatedNew)/',$_SERVER['REQUEST_URI']))?'in':'' }}">
            <div class="g-sidenav {{ (preg_match('/^\/user\/(productionUploading|productionFeatedNew)/',$_SERVER['REQUEST_URI']))?'z-active':'' }}">
                <a href="/user/productionUploading" class="g-wrap2 {{ (preg_match('/^\/user\/(productionUploading|productionFeatedNew)/',$_SERVER['REQUEST_URI']))?'active':'' }}">作品上传</a>
            </div>
        </div>
    </div>
    <div class="panel panel-default">
        <div class="panel-heading">
            <h4 class="panel-title clearfix">
                ...
                {{ (preg_match('/^\/user\/(acceptTasksList|serviceMyJob|workComment|mySellGoods)/',$_SERVER['REQUEST_URI']))?'g-active':'' }} collapsed">
        ...
        <div id="collapseThree4" class="panel-collapse collapse {{ (preg_match('/^\/user\/(acceptTasksList|serviceMyJob|workComment|mySellGoods)/',$_SERVER['REQUEST_URI']))?'in':'' }}">
            <div class="g-sidenav {{ (preg_match('/^\/user\/(myTask|acceptTasksList)/',$_SERVER['REQUEST_URI']))?'z-active':'' }}">
                <a href="/user/acceptTasksList" class="g-wrap2 {{ (preg_match('/^\/user\/(myTask|myTasksList)/',$_SERVER['REQUEST_URI']))?'active':'' }}">我承接的项目</a>
            </div>
    ...
    <div class="panel panel-default">
        <div class="panel-heading">
            <h4 class="panel-title clearfix">
                <a href="#collapseThrees2" data-parent="#accordion" data-toggle="collapse" class="accordion-toggle g-wrap1
                   {{ (preg_match('/^\/user\/(switchUrl|shop|vipshopbar|myShopSuccessCase|goodsShop|serviceList|serviceEdit|serviceCreate|serviceBounty|shopcommentowner|waitGoodsHandle|pubGoods|editGoods|addShopSuccess|editShopSuccess|enterpriseAuth)/',$_SERVER['REQUEST_URI']))?'g-active':'' }} collapsed">
        ...
        {{ (preg_match('/^\/user\/(switchUrl|shop|vipshopbar|myShopSuccessCase|goodsShop|serviceList|serviceEdit|serviceCreate|serviceBounty|shopcommentowner|waitGoodsHandle|pubGoods|editGoods|addShopSuccess|editShopSuccess|enterpriseAuth|waitServiceHandle|vipshopbar)/',$_SERVER['REQUEST_URI']))?'in':'' }}">
        ...
        <div class="panel-heading">
            <h4 class="panel-title clearfix">
                <a href="#collapseThrees5" data-parent="#accordion" data-toggle="collapse" class="accordion-toggle g-wrap1
                   {{ (preg_match('/^\/user\/(vippaylist|vippaylog)/',$_SERVER['REQUEST_URI']))?'g-active':'' }} collapsed">
