...
*/
    /*var myreg=/^((13[0-9])|(14[5,7])|(15[0-3,5-9])|(17[0,3,5-8])|(18[0-9])|166|198|199|(147))\\d{8}$/;
     if(myreg.test(mobile))
     {
         ...
         $.post('/user/checkEmail',{'_token':token,'mobile':mobile},function(data){
             if(data.errCode==1){
                 sendMessage(mobile);
             }else{
...
/*$('#sendMobileCode').click(function()
{
    ...
    {
        var token = $('#form-field-3').attr('token');
        $.post('/user/checkMobile',{'_token':token,'mobile':mobile},function(data){
            if(data.errCode==1){
                sendMessage(mobile);
            }else{
                $('#sendMobileCode').next('div').attr('class','Validform_checktip Validform_wrong');
