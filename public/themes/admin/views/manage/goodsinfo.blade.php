...
<form class="form-horizontal" action="/manage/saveGoodsInfo" method="post" name="seo-form">
    ...
    <div class="g-backrealdetails clearfix bor-border interface">
        ...
        <div class="space-8 col-xs-12"></div>
            ...
            <div class="form-group interface-bottom col-xs-12">
                <label class="col-sm-1 control-label no-padding-right" for="form-field-1"> 作品封面</label>
                <div class="col-sm-9">
                    <label class="col-sm-2 row"><img src="{{ossUrl($goods_info->cover)}}" /></label>
                    <label class="col-sm-12 row"><a href="{{ossDownloadUrl($goods_info->cover)}}" >下载</a></label>
                </div>
            </div>
            <div class="form-group interface-bottom col-xs-12">
                <label class="col-sm-1 control-label no-padding-right" for="form-field-1"> 作品</label>
                <div class="col-sm-9">
                    @if($goods_info->file != "" && json_decode($goods_info->file))
                    @foreach(json_decode($goods_info->file) as $file)
                        @if($file)
                        <label class="col-sm-2 row"><img src="{{ossUrl($file)}}" /></label>
                        <label class="col-sm-12 row"><a href="{{ossDownloadUrl($file)}}" >下载</a></label>
                        @endif
                    @endforeach
                    @else
                        <label class="col-sm-4 control-label no-padding-right" style="color: #CC0000"> 没有图片或者图片多余14张，让用户重新上传</label>
                    @endif
                </div>
            </div>
            <div class="form-group interface-bottom col-xs-12">
